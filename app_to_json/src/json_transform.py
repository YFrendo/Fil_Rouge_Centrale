...
def txt_json(path):  # Va transformer le fichier texte en JSON
    ...
    try:
        for (
            ...
            fichier
        ):  # Parcours le fichier pour compter le nombre de ligne et de caractère ainssi que les mettres dans une variable string
            nb_ligne += 1
            nb_carac += len(ligne)
            texte = texte + ligne
    ...
    metadata = {
        "taille": taille,
        "MIME": "txt",
        "NombreLigne": nb_ligne,
        "nombre-caractère": nb_carac,
    }  # Création du dictionnaire de métadonné
...
def detect_labels_rekognition(
    path,
):  # Cette fonction permet de demandé à AWS de faire de la reconnaissance d'image
    with open(path, "rb") as f:
        ...
        )  # Je ne séléctionne que les labels avec une confience de 95% ou plus
        return response
...
def image_json(path):  # Images en JSON
    exifTag = (
        ...
        "Model",
        "Orientation",
    )  # Liste des tag pour les métadonnées
    ...
    data["img"] = base64_bytes.decode(
        "utf-8"
    )  # On transforme la base64 en string pour pouvoir la stocker dans un JSON
    image = Image.open(
        path
    )  # Si l'image ne peuit pas etre ouverte par PIL alors le programme plante ce qui est récupérer dans le programme principal
